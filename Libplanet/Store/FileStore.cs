...
namespace Libplanet.Store
{
    public class FileStore : IStore
    {
        ...
        public string GetBlockPath(HashDigest blockHash)
        {
        ...
        public override long AppendIndex(HashDigest hash)
        {
                ...
                FileMode.Append, FileAccess.Write))
            {
                ...
                return offset / HashDigest.HashSize;
            }
        }
        ...
        public override long CountIndex()
        {
            ...
            if (indexFile.Exists)
            {
                ...
                if (indexSize % HashDigest.HashSize != 0)
                {
                    throw new FileLoadException(
                        $"Index size {indexSize} should be a multiple " +
                        $"of HashSize {HashDigest.HashSize}"
                    );
                }

                return indexSize / HashDigest.HashSize;
            }

            return 0;
        }

        public override bool DeleteBlock(HashDigest blockHash)
        {
        ...
        public override Block<T>? GetBlock<T>(HashDigest blockHash)
        {
            var blockFile = new FileInfo(GetBlockPath(blockHash));
            if (!blockFile.Exists)
            {
            ...
            using (Stream stream = blockFile.OpenRead())
            {
                ...
                HashDigest? previousHash = (rawBlock.PreviousHash != null) ?
                    new HashDigest?(new HashDigest(rawBlock.PreviousHash)) :
                    null;
                return new Block<T>(
        ...
        public override HashDigest? IndexBlockHash(long index)
        {
            if (index < 0)
            {
                index += CountIndex();
            ...
            {
                var blockHash = new byte[HashDigest.HashSize];
                stream.Seek(HashDigest.HashSize * index, SeekOrigin.Begin);
                var bytesRead = stream.Read(blockHash, 0, HashDigest.HashSize);
                Trace.Assert(bytesRead == HashDigest.HashSize);

                return new HashDigest(blockHash);
            }
        }
        ...
        public override IEnumerable<HashDigest> IterateBlockHashes()
        {
            var prefixRegex = new Regex(@"^[a-f0-9]{4}$", RegexOptions.IgnoreCase);
            ...
            foreach (DirectoryInfo prefix in addressesRoot.EnumerateDirectories())
            {
                ...
                foreach (FileInfo rest in prefix.EnumerateFiles())
                {
                    ...
                    yield return new HashDigest(ByteUtil.ParseHex(prefix.Name + rest.Name));
                }
            }
        }

        public override IEnumerable<HashDigest> IterateIndex()
        {
            if (!File.Exists(_indexPath))
            {
                yield break;
            ...
            using (Stream stream = IndexFileStream())
            {
                while (true)
                {
                    ...
                    yield return new HashDigest(blockHash);
                }
            }
        }
        ...
        public override States GetBlockStates(HashDigest blockHash)
        {
            ...
            using (Stream stream = statesFile.OpenRead())
            {
                ...
                return (States)formatter.Deserialize(stream);
            }
        }
        ...
        private FileStream IndexFileStream()
        {
            ...
            if (stream.Length % HashDigest.HashSize != 0)
            {
                throw new FileLoadException(
